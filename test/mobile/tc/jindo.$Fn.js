...
module("$Fn 객체");
	...
	QUnit.test("attach : 이벤트 추가시 gc.pool의 크기가 증가하는가?",function(){
		...
		$Fn(function() { }).attach(document, 'click');
	...
	QUnit.test("usecapture은 정상적으로 적용이 되는가?",function(){
		var fn = $Fn(function(){});
...
//		_ua = __real_ua;
//	},
//	QUnit.test("unload 이벤트는 정상적으로 동작해야 한다.",function(){
...
//
//	},
	QUnit.test("freeElement을 직접 호출할때는 호출되면 안된다.",function(){
		var isExcuteUnLoad = false;
		var isExcuteUnLoad2 = false;
		var fp = $Fn(function(){
			isExcuteUnLoad = true;
	...
	QUnit.test("attach의 스코프는 정상적이어야 한다.",function(){
		...
		var test = {
			...
			"scope" : function(){
				callCount++;
				value_of(this.key).should_be("test");
			}
		}
	...
	*/
	...
	QUnit.test("animation이벤트나 transition 이벤트를 사용할수 있도록 수정.",function(){
		var _sName1,_sName2,_sName3,_sName4,_sName5;
		if(typeof document.body.style.WebkitTransition !== "undefined"){
			_sName1 = "webkitTransitionStart";
			_sName2 = "webkitTransitionEnd";
		}else if(typeof document.body.style.OTransition !== "undefined"){
			_sName1 = "oTransitionStart";
			_sName2 = "oTransitionEnd";
		}else if(typeof document.body.style.MsTransition !== "undefined"){
			_sName1 = "msTransitionStart";
			_sName2 = "msTransitionEnd";
		}else{
			_sName1 = "transitionStart";
			_sName2 = "transitionEnd";
		}

		if(typeof document.body.style.WebkitAnimationName !== "undefined"){
			_sName3 = "webkitAnimationStart";
			_sName4 = "webkitAnimationEnd";
			_sName5 = "webkitAnimationIteration";
		}else if(typeof document.body.style.OAnimationName !== "undefined"){
			_sName3 = "oAnimationStart";
			_sName4 = "oAnimationEnd";
			_sName5 = "oAnimationIteration";
		}else if(typeof document.body.style.MsTransitionName !== "undefined"){
			_sName3 = "msAnimationStart";
			_sName4 = "msAnimationEnd";
			_sName5 = "msAnimationIteration";
		}else{
			_sName3 = "animationStart";
			_sName4 = "animationEnd";
			_sName5 = "animationIteration";
		}
		equal(jindo.$Element.eventManager.revisionEvent("normal","transitionStart"),_sName1);
		equal(jindo.$Element.eventManager.revisionEvent("normal","transitionEnd"),_sName2);
		equal(jindo.$Element.eventManager.revisionEvent("normal","animationStart"),_sName3);
		equal(jindo.$Element.eventManager.revisionEvent("normal","animationEnd"),_sName4);
		equal(jindo.$Element.eventManager.revisionEvent("normal","animationIteration"),_sName5);
	});

module("$Fn 객체 NEW API");
	QUnit.test("$Fn의 생성자는 문자이거나 함수여야 한다.",function(){
		equal($Fn(true),null);
		equal($Fn({}),null);
	});

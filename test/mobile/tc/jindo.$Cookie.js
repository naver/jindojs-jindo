...
document.cookie = "nickname="+escape("gony")+(";expires="+(new Date((new Date()).getTime()-(1*1000*60*60*24))).toGMTString())+"; path=/";
document.cookie = "blog="+escape("http://mygony.com")+(";expires="+(new Date((new Date()).getTime()-(1*1000*60*60*24))).toGMTString())+"; path=/";
module("$Cookie 객체");
	...
	QUnit.test("set : 쿠키가 설정되나?",function(){
		var c = $Cookie();
		ok(c.set('nickname', 'gony',10) === c);
		var coo = (document.cookie)+"";
		var sDomain = document.domain;
		ok(/nickname=gony/.test(coo));
		ok(c.set('nickname2', 'gony2',0, sDomain) === c);
		ok(c.set('nickname4', 'gony4',0, sDomain, "/test") === c);
	...
	QUnit.test("get : 쿠키를 가져오나?",function(){
		...
		equal(c.get('nickname'),'netil');
		equal(c.get('nickname3'),null);
	});

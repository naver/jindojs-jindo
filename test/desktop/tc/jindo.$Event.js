...
module("$Event");
	...
	QUnit.test("changedTouch, targetTouch을 실행하면 에러가 발생해야 한다.",function(){
		var wEvent1 = $Event({
			"type" : "touchstart",
			"target" : document,
			"currentTarget" : document,
    ...
    QUnit.test("hook은 정상적으로 삭제한다.",function(){
        ...
        //Then
        deepEqual(jindo.$Event.hook("test"), undefined);
        deepEqual(jindo.$Event.hook("test1"), undefined);
        deepEqual(jindo.$Event.hook("test2"), undefined);
    }
);
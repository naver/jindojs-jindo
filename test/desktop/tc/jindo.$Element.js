...
jindo.$Event.customEvent("someCustomEvent");
jindo.$Element("custom_child").attach("someCustomEvent",customChild);
jindo.$Element("custom_parent").attach("someCustomEvent",customParent);
 ...
 module("$Event#customEvent에서 두 번째 인자가 없어도 ", {
    setup: function() {
     ...
     QUnit.test("실행시킬 수 있어야 한다.",function(){
         //Given
         //When
         jindo.$Element("custom_parent").fireEvent("someCustomEvent");
         //Then
     ...
     QUnit.test("버블링이 되어야 한다.",function(){
         //Given
         //When
         jindo.$Element("custom_child").fireEvent("someCustomEvent");
         //Then
     ...
     QUnit.test("정상적으로 detach되어야 한다.",function(){
         //Given
         //When
         jindo.$Element("custom_parent").detach("someCustomEvent",customParent);
         jindo.$Element("custom_child").detach("someCustomEvent",customChild);
...
var IS_LOW_IE9 = _JINDO_IS_IE && parseInt(_j_ag.match(/(?:MSIE) ([0-9.]+)/)[1]) < 9;
var IS_IE8 = _JINDO_IS_IE && parseInt(_j_ag.match(/(?:MSIE) ([0-9.]+)/)[1]) == 8;
...
module("$Element 객체");
    ...
    QUnit.test("offset : 위치값을 잘 설정합니까?",function(){
        ...
        var offset1 = rel.offset();

        deepEqual({
                top: Math.floor(offset1.top),
                left: Math.floor(offset1.left)
            }, {
                top: Math.floor(rel_pos.top + 10),
                left: Math.floor(rel_pos.left + 30)
        });

        var abs_pos = abs.offset();
        abs.offset(abs_pos.top + 10, abs_pos.left - 5);

        var offset2 = abs.offset();
        deepEqual({
            top: Math.floor(offset2.top),
            left: Math.floor(offset2.left)
        }, {
            top: Math.floor(abs_pos.top + 10),
            left: Math.floor(abs_pos.left - 5)
        });
...
//
    ...
    QUnit.test("removeClass는 class를 삭제 해야 한다.",function(){
    ...
    QUnit.test("rgb는 정상적으로 반환하는가?",function(){
        ...
        if ((_JINDO_IS_IE && parseInt(_j_ag.match(/(?:MSIE) ([0-9.]+)/)[1]) < 9)) {
            equal($Element("class_api_update").css("backgroundColor"),"#ffffff");
        }
        else {
    ...
    if(jindo.$Agent().navigator().ie && jindo.$Agent().navigator().version < 9) {
        QUnit.test("$Element의 show에서 catch구문을 태우기.",function(){
            $Element("ele_try").show("test");
            equal($Element("ele_try").css("display"),"block");
        });
    }

    QUnit.test("CSS rule로 설정된 $Element의 opacity는 정상적으로 반환 되어야 한다.",function(){
        $Element("ele_opacity").opacity("");
    ...
    QUnit.test("$Element의 opacity에 문자가 들어간  경우는 기존 값을 그대로 유지한다.",function(){
        $Element("ele_opacity").opacity(1);
    ...
    QUnit.test("$Element의 css에 첫번째 인자는 문자나 오브젝트가 아닌 경우는 exception이 발생한다.",function(){
        ...
        if ((_JINDO_IS_IE && parseInt(_j_ag.match(/(?:MSIE) ([0-9.]+)/)[1]) > 6)) {
            occurException = false;
            try {
                $Element("ele_opacity").css(1);
            }
            catch (e) {
                occurException = true;
            }
    ...
    QUnit.test("outerHTML은 정상적으로 작동해야 한다.",function(){
        var compare_str;
        if (IS_LOW_IE9) {
            compare_str = "\r\n<DIV ID=OUTER_HTML><SPAN>1</SPAN></DIV>";
        }
        else
            if (navigator.userAgent.indexOf("MSIE") > -1) {
                compare_str = "<DIV ID=\"OUTER_HTML\"><SPAN>1</SPAN></DIV>";
            }
            else {
                compare_str = "<div id=\"outer_html\"><span>1</span></div>".toUpperCase();
            }
        equal($Element("outer_html").outerHTML().toUpperCase().replace(/(\s__jindo__id=\"(?:.*?)\")/i, function(){
            return "";
        }), compare_str);
    });
    ...
    QUnit.test("beforeHTML",function(){
        ...
        equal($("beforeInnerHTML1").innerHTML.replace(/(\s__jindo__id=\"(?:.*?)\")/i, function(){
            return "";
        }), $("admock").innerHTML);
    });
    QUnit.test("afterHTML",function(){
        ...
        equal($("afterInnerHTML2").innerHTML.replace(/(\s\_\_jindo\_\_id=\"(?:.*?)\")/i, function(){
            return "";
        }), $("admock").innerHTML);
    });
        // QUnit.test(".ch1인 엘리먼트가 클릭이 되면 이벤트는 발생되어야 한다.",function(){
...
//
    ...
    QUnit.test("wrap는 정상적으로 동작해야 한다.",function(){
        ...
        QUnit.test("queryAll는 정상적으로 동작해야 한다.",function(){
            equal($Element(document).queryAll("#ele_query_test li").length,2);
        });
        QUnit.test("query는 정상적으로 동작해야 한다.",function(){
            equal($Element(document).query("#ele_query_test li").$value().tagName.toUpperCase(),"LI");
        });
...
/*    QUnit.test("IE에서 DOM에 append하기 전에 opacity을 설정했을 때 생기는 문제.",function(){
        try {
            ...
            value_of().should_fail(e.message);
        }
    });*/
    ...
    QUnit.test("delegate에서 이벤트가 전파되어야 한다.",function(){
        ...
        equal(clickedEle,$$.getSingle("#a_a"))
    });
    ...
    QUnit.test("filter가 function일때도 정상적으로 동작해야 한다.",function(){
        ...
        equal(clickedEle,$$.getSingle("#a_a3"))
    });
...
//
    ...
    QUnit.test("document에 'load' 이벤트를 attach 한 후, hasEventListener()를 호출하면 true를 반환해야 한다. ",function(){
...
module("cache 문제");
    ...
    if(!document.querySelectorAll) {
        QUnit.test("html을 이용했을떼 css cache가 비워져 있는가?",function(){
        ...
        QUnit.test("replace을 이용했을떼 css cache가 비워져 있는가?",function(){
        ...
        QUnit.test("empty을 이용했을떼 css cache가 비워져 있는가?",function(){
            $Element("cssqueryrealse").html("<a href='1'>aaa</a>");
            cssquery.useCache(true);
            cssquery._resetUID();
            cssquery.release();
            ...
            $Element("cssqueryrealse").html('<a href="#test">test</a>');
            $Element("cssqueryrealse").query("a");
        ...
        QUnit.test("remove을 이용했을떼 css cache가 비워져 있는가?",function(){
            $Element("cssqueryrealse").html("<a href='1'>aaa</a>");
            cssquery.useCache(true);
            cssquery._resetUID();
            cssquery.release();
        ...
        QUnit.test("leave을 이용했을떼 css cache가 비워져 있는가?",function(){
            $Element("cssqueryrealse").html("<a href='1'>aaa</a>");
            cssquery.useCache(true);
            cssquery._resetUID();
            cssquery.release();
...
module("$Element New API");
    ...
    QUnit.test("Metacharacter가 포함된 클래스도 올바로 제거 되어야 한다.",function(){
        var ele = $Element("double_class"),
            aClass = [ "list", "[abc]", "^abc:", "[abc]a", "_test", "list1", "list2", "_test:('dkdkd')" ];
...
//
    ...
    QUnit.test("IE8~9에서 href의 값이 오브젝트여서 문제가 발생가 발생.",function(){
        //Given
        var href = $("anchor").href;
        var weEle = $Element("<img>");
        //When
        weEle.attr("src",href);
        //Then
        equal(weEle.attr("src"),"http://static.naver.com/img/some.jpg");
    });
    QUnit.test("IE9에서 padding값이 정확하지 않음.[[!IS_IE8]]",function(){
        //JINDOSUS-721 참고.
        //Given
        var rich = $Element("paddingBug");
        //Then
        var px = parseInt(rich.css("paddingLeft"),10);
        //Then
        equal(px,0);
    });
...
QUnit.test("data메서드는 기본 기능은 정상적으로 동작해야 한다.",function(){
    ...
    QUnit.test("data메서드는 dash와 camelCase모두 get할 수 있다.",function(){
        //Given
        //When
        var value = $Element("dataHello").data("hello-world");
        var value2 = $Element("dataHello").data("helloWorld");
        //Then
        equal(value,"world");
        equal(value2,"world");
    });
    ...
    QUnit.test("$Element에 textnode가 들어가도 에러가 발생하면 안된다.",function(){
        ...
        //Then
        try{
            var  ele = jindo.$Element("<div>").append(document.createTextNode("\n"));
        }catch(e){
            raiseError = true;
        }
    ...
    QUnit.test("svg에서 classList을 사용할 수 없다면 jindo._p_.canUseClassList 는 false여야 한다.",function(){
    ...
    QUnit.test("\\이 들어가면 정상적으로 나와야 한다.",function(){
        //Given
        //When
        $Element("datatest3").data("type", "asdf");
        var type = $Element("datatest3").data("type");
        //Then
        equal(type,"asdf");
    ...
    QUnit.test("hook은 정상적으로 셋팅/반환한다.",function(){
        //Given
        //When
        jindo.$Element.hook("test_hook","Test");

        //Then
        equal(jindo.$Element.hook("test_hook"),"Test");

        //Given
        //When
        jindo.$Element.hook("test_hook",function(){return "Test"});
        ...
        //Then
        equal(jindo.$Element.hook("test_hook1")(),"Test1");
        equal(jindo.$Element.hook("test_hook2"),"Test2");
    ...
    QUnit.test("hook은 정상적으로 삭제한다.",function(){
        //Given
        //When
        jindo.$Element.hook("test_hook",null);
        jindo.$Element.hook("test_hook1",null);
        jindo.$Element.hook("test_hook2",null);

        //Then
        deepEqual(jindo.$Element.hook("test_hook"), undefined);
        deepEqual(jindo.$Element.hook("test_hook1"), undefined);
        deepEqual(jindo.$Element.hook("test_hook2"), undefined);
    });
    ...
    QUnit.test("delegate로 이벤트가 발생했을 때 바인딩한 함수의 이벤트는 서로 다른 $Event객체여야 한다",function(){
        ...
        //Then
        notEqual(firstEvent, secondEvent);
    });

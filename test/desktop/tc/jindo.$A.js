...
module("$A 객체");
	...
	QUnit.test("length : 배열의 크기",function(){
		deepEqual($A().length(),0);
		deepEqual($A([1,2,3]).length(),3);
	...
	QUnit.test("indexOf : 주어진 원소가 배열의 몇번째 요소인가",function(){
		deepEqual($A([1,2,3]).indexOf(2),1);
		deepEqual($A([1,2,3]).indexOf(3),2);
		deepEqual($A([1,2,3]).indexOf(0),-1);
	});
	QUnit.test("push : 배열에 원소 추가",function(){
		var a1 = $A([1,2,3]);

		deepEqual(a1.push('a'),4);
		deepEqual(a1.push('b','c'),6);
	...
	QUnit.test("unshift : 배열 앞부분에 원소 삽입",function(){
		...
		deepEqual(a.unshift('c'),3);
		deepEqual(a.$value(),['c',4,5]);
		deepEqual(a.unshift('a','b'),5);
	...
	QUnit.test("toString : 원래의 배열처럼 잘 표현되나?",function(){
		...
		deepEqual(a.toString(),arr.toString());
	});
...
*/
	...
	QUnit.test("concat는 2개의 합쳐진 새로운 배열을 반환하는가?",function(){
	    ...
	    deepEqual(arr3.length(),6);
	    ok(!(arr1.$value() == arr3.$value()));
	    deepEqual(arr1.concat([4,5,6]).length(),6);
	    deepEqual(arr1.concat().length(),3);
	    deepEqual(arr1.concat(null).length(),3);
        deepEqual(arr1.concat(arr2, [7,8,9], "test").length(),10);
	});
...
module("$A new api");
	...
	QUnit.test("생성자는 배열, $A, null이거나 없어야 한다.",function(){
		...
		if ((!navi.ie) || navi.version > 6) {
			deepEqual($A("asdf"),null);
			deepEqual($A({}),null);
			deepEqual($A(true),null);
			deepEqual($A(1),null);
		}

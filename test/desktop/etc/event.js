 ...
 * @param {Object} thisHash 콜백 함수의 this
 * @example
 ...
 */
jindo.$H.prototype.forEach = function(callback, thisHash) {
	var t = this._table;
	...
	for(var k in t) {
		...
		try {
			callback.call(thisHash, t[k], k, t);
		} catch(e) {
 ...
 */
jindo.$H.prototype.filter = function(callback, thisHash) {
	var h = jindo.$H();
	this.forEach(function(v,k,o) {
		if(callback.call(thisHash, v, k, o) === true) {
			h.add(k,v);
		}
	});
	return h;
 ...
 */
jindo.$H.prototype.map = function(callback, thisHash) {
	var t = this._table;
	this.forEach(function(v,k,o) {
		t[k] = callback.call(thisHash, v, k, o);
	});
	return this;
};
 ...
 * @param {Object | String} thisHash 함수 객체가 다른 객체의 메서드일 때, 해당 객체도 같이 전달한다. 혹은 함수 몸체를 지정하는 문자열.
 * @return {$Fn} $Fn 객체
 * @see $Fn#toFunction
 * @description [Lite]
 * @example
 ...
 * @author Kim, Taegon
 */
jindo.$Fn = function(func, thisHash) {
	var cl = arguments.callee;
	if (func instanceof cl) return func;
	if (!(this instanceof cl)) return new cl(func, thisHash);
	...
	if (typeof func == "function") {
		this._func = func;
		this._this = thisHash;
	} else if (typeof func == "string" && typeof thisHash == "string") {
		this._func = new Function(func, thisHash);
	}
}

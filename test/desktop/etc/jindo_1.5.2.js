	...
	for(var i=0; i < nArgLeng; i++) {
		...
		if (typeof el == "string") {
			...
			if (el.indexOf("<")>-1) {
				...
				el = jindo._getElementById(doc,el);
			}
		}
...
jindo._getElementById = function(doc,id){
	/*
	 * ie6,7에서 name으로도 getElementById를 사용할 수 되어 있어 수정. 
	 */
			...
			case ' ':
				if (oExpr.quotID) {
					sTmpFunc +=
						...
						'var oEl = jindo._getElementById(oDocument_dontShrink,' + oExpr.quotID + ');' +
						'var oCandi = oEl;' +
...
jindo.$Ajax.SWFRequest = jindo.$Class({
	...
	_getFlashObj : function(){
		...
		if (navi.ie&&navi.version==9) {
			obj = jindo._getElementById(document,jindo.$Ajax.SWFRequest._tmpId);
		}else{
...
jindo.$Template = function(str) {
    ...
    if(typeof str == "undefined") {
		str = "";
	}else if( (obj=jindo._getElementById(document,str)||str) && obj.tagName && (tag=obj.tagName.toUpperCase()) && (tag == "TEXTAREA" || (tag == "SCRIPT" && obj.getAttribute("type") == "text/template")) ) {
